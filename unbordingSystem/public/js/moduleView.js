...
function loadModuleDetails(moduleId) {
    fetch(`..\..\src\controller\getModuleDetails.php?moduleId=${moduleId}`)
        ...
        .then(data => {
            ...
            const questionsHtml = data.questions.map(q => `
                <div class="question">
                    ...
                    <textarea id="answer-${q.id}" placeholder="Enter your answer here"></textarea>
                </div>
            `).join('');
...
function submitModule() {
    ...
    fetch('..\..\src\controller\submitModule.php', {
        method: 'POST',
    ...
    .then(data => {
        ...
        else alert("Error submitting module");
    })
